...
const MAX_RFID_POWER_FOR_THE_EU: number = 31.5; // dBm
...
export class GetLlrpMessage {
  ...
  public static addRoSpec(roSpecID: number, parameters: RospecParameters): Buffer {
    ...
    const rOSpecStartTrigger: Buffer = this.makeTLVParameter(
      parameterC.ROSpecStartTrigger,
      new Buffer([rospecConstants.ROSpecStartTriggerType])
    );
    ...
    if (сonfigEachAntenna) {
      ...
      parameters.antennasConfig.forEach((config: AntennaConfig, index: number) => {
    ...
    let antennaConfiguration: Buffer = GetLlrpMessage.getAntennaConfig(
      0,
    ...
    const inventoryParameterSpec: Buffer = this.makeTLVParameter(
      parameterC.InventoryParameterSpec,
    ...
    const c1G2EPCMemorySelector: Buffer = this.makeTLVParameter(
      parameterC.C1G2EPCMemorySelector,
    ...
    const tagReportContentSelector: Buffer = this.makeTLVParameter(
      parameterC.TagReportContentSelector,
  ...
  static getAntennaConfig(
    ...
    const c1g2SingulationControl: Buffer = this.makeTLVParameter(
      parameterC.C1G2SingulationControl,
    ...
    const c1g2InventoryCommand: Buffer = this.makeTLVParameter(
      parameterC.C1G2InventoryCommand,
    ...
    const antennaConfiguration: Buffer = this.makeTLVParameter(
      parameterC.AntennaConfiguration,
      Buffer.concat([antennaConfigurationParam, rfReceiver, rfTransmitter, c1g2InventoryCommand])
    );
